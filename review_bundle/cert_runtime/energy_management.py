...
class EnergyManagementTransition:
    observation: np.ndarray
    ...
    def __post_init__(self) -> None:
        ...
        if observation.ndim != 1 or next_observation.shape != observation.shape:
            raise ValueError("energy-management observations must be matching vectors")
        if self.duration_steps <= 0:
            raise ValueError("energy-management SMDP duration must be positive")
        if self.requested_decision not in (0, 1) or self.executed_decision not in (0, 1):
            raise ValueError("energy-management decisions must be binary")
        if not self.scenario_id or not self.manifest_hash:
            raise ValueError("energy-management replay requires scenario and manifest binding")
        object.__setattr__(self, "observation", observation)
        object.__setattr__(self, "next_observation", next_observation)


class EnergyManagementReplayBuffer:
    def __init__(self, capacity: int, scenario_manifests: dict[str, str] | None = None) -> None:
        if capacity <= 0:
            raise ValueError("energy-management replay capacity must be positive")
        self.capacity = capacity
        self.scenario_manifests = dict(scenario_manifests or {})
        self.records: list[EnergyManagementTransition] = []

    def add(self, transition: EnergyManagementTransition) -> None:
        expected = self.scenario_manifests.get(transition.scenario_id)
        if expected is not None and transition.manifest_hash != expected:
            raise ValueError("energy-management scenario/manifest mismatch")
        self.records.append(EnergyManagementTransition(
    ...
    def sample(self, batch_size: int, rng: np.random.Generator) -> list[EnergyManagementTransition]:
        if batch_size <= 0 or len(self.records) < batch_size:
            raise ValueError("insufficient energy-management replay")
        groups: dict[str, list[EnergyManagementTransition]] = {}
        ...
        if not eligible:
            raise ValueError("no manifest-compatible energy-management batch")
...
class ReserveOnlyEnergyPolicy:
    def select_action(self, observation: np.ndarray, context: dict, deterministic: bool = False) -> EnergyDecision:
        del observation, deterministic
        if context.get("charging", False):
            return EnergyDecision.SERVE_OR_LEAVE if context.get("departure_allowed", False) else EnergyDecision.CHARGE_OR_STAY
        return EnergyDecision.SERVE_OR_LEAVE


class FixedThresholdEnergyPolicy:
    ...
    def select_action(self, observation: np.ndarray, context: dict, deterministic: bool = False) -> EnergyDecision:
        ...
        if context.get("charging", False):
            ...
            return EnergyDecision.SERVE_OR_LEAVE if leave else EnergyDecision.CHARGE_OR_STAY
        return EnergyDecision.CHARGE_OR_STAY if energy_fraction < self.threshold else EnergyDecision.SERVE_OR_LEAVE


class FullChargeEnergyPolicy(FixedThresholdEnergyPolicy):
...
@dataclass(frozen=True, slots=True)
class EnergyManagementSACConfig:
    gamma: float = 0.99
    tau: float = 0.005
    actor_lr: float = 3e-4
...
class EnergyManagementSAC:
    """Discrete soft actor-critic over event-to-event energy decisions."""
    ...
    def __init__(
        self,
        observation_dim: int,
        config: EnergyManagementSACConfig | None = None,
        seed: int = 0,
        device: str = "cpu",
    ) -> None:
        self.config = config or EnergyManagementSACConfig()
        ...
        self.replay = EnergyManagementReplayBuffer(self.config.replay_capacity)
    ...
    def observe(self, transition: EnergyManagementTransition) -> None:
    ...
    def update(self) -> dict[str, float | str]:
        ...
        return {
            ...
            "energy_management_density": "categorical-softmax; independent of Generator affine-tanh density",
        }


def make_energy_management_policy(name: str, observation_dim: int | None = None, **kwargs):
    if name == "reserve_only":
        return ReserveOnlyEnergyPolicy()
    if name == "fixed_threshold_30":
        return FixedThresholdEnergyPolicy(0.30)
    if name == "fixed_threshold_50":
        return FixedThresholdEnergyPolicy(0.50)
    if name == "full_charge":
        return FullChargeEnergyPolicy()
    if name in {"energy_management_sac", "scheduler_sac"}:
        if observation_dim is None:
            raise ValueError("energy_management_sac requires observation_dim")
        return EnergyManagementSAC(observation_dim, **kwargs)
    raise ValueError(f"unknown energy-management policy: {name}")
