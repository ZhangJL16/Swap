...
def main() -> None:
    ...
    policy = env.energy_management_policy
    updates = []
    for step in range(args.steps):
        _, _, terminated, truncated, _ = env.step(np.zeros(3, dtype=np.float64))
        if len(policy.replay.records) >= policy.config.batch_size:
            updates.append({"step": step, **policy.update()})
    ...
    torch.save({
        ...
        "critic_1": policy.critic_1.state_dict(),
        "critic_2": policy.critic_2.state_dict(),
        "log_alpha": policy.log_alpha.detach().cpu(),
    }, output / "checkpoint_latest.pt")
    (output / "metrics.json").write_text(
        json.dumps({"updates": updates, "metrics": env.metric_snapshot()}, indent=2),
        encoding="utf-8",
    )
