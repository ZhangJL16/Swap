...
for seed in 0 1 2; do
  [[ ! -e "artifacts/phase1_sb3_ddpg_1m/seed${seed}/config.json" ]] || { echo "existing DDPG run seed${seed}" >&2; exit 3; }
done
if [[ "$run_mode" == "serial" ]]; then
  session="sb3-ddpg1m-queue"
  tmux new-session -d -s "$session" -c "$root" \
    "export CUDA_VISIBLE_DEVICES=$physical_gpu_index DEVICE=${DEVICE:-auto}; for seed in 0 1 2; do bash scripts/run_one_sb3_ddpg_1m.sh \$seed artifacts/phase1_sb3_ddpg_1m/seed\$seed || exit \$?; done"
  echo "launched serial DDPG queue session=$session"
  ...
  for seed in 0 1 2; do
    ...
    session="sb3-ddpg1m-s${seed}"
    tmux new-session -d -s "$session" -c "$root" "export CUDA_VISIBLE_DEVICES=$gpu DEVICE=${DEVICE:-auto}; bash scripts/run_one_sb3_ddpg_1m.sh $seed artifacts/phase1_sb3_ddpg_1m/seed$seed"
    echo "launched DDPG seed=$seed session=$session physical_gpu=$gpu"
  done
else
