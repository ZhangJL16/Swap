...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run persistent charging-scheduler baselines (manual experiment entry point).")
    ...
    parser.add_argument("--output", default="artifacts/persistent/baselines.json")
    args = parser.parse_args()
    rows = []
    for scenario in args.scenarios:
        for method in args.methods:
            for seed in args.seeds:
                env = make_persistent_uav_env(f"{scenario}.json", scheduler_name=method, seed=seed, timing_mode="functional")
                env.reset(seed=seed)
