...
class ReplayBuffer:
    def __init__(self, args):
        ...
        self.raw_obs_shape = getattr(self.args, "raw_obs_shape", self.obs_shape)
        self.size = self.args.buffer_size
        self.episode_limit = self.args.episode_limit
        self.store_raw_obs = (
            (
                self.args.alg.find("vdn") != -1
        ...
        self.buffers = {
            ...
            's_next': np.empty([self.size, self.episode_limit, self.state_shape], dtype=np.float32),
            'avail_u': np.empty([self.size, self.episode_limit, self.n_agents, self.n_actions], dtype=np.float32),
            'avail_u_next': np.empty([self.size, self.episode_limit, self.n_agents, self.n_actions], dtype=np.float32),
            'u_onehot': np.empty([self.size, self.episode_limit, self.n_agents, self.n_actions], dtype=np.float32),
        ...
        if self.store_raw_obs:
            self.buffers['o_raw'] = np.empty(
                [self.size, self.episode_limit, self.n_agents, self.raw_obs_shape],
                dtype=np.float32,
            )
            self.buffers['o_next_raw'] = np.empty(
                [self.size, self.episode_limit, self.n_agents, self.raw_obs_shape],
                dtype=np.float32,
            )
