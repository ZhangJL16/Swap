...
class UAVEnv:
    ...
    def _initialize_goals(self):
        self.goals = []
        for goal_idx in range(self.num_agents):
            for _ in range(self.sample_retry_limit):
                goal = self._sample_position(lower_margin=0.3)
                candidate_goal = GoalPoint(goal)
                if self._is_valid_goal(goal, self.goals, self.agents):
                    self.goals.append(candidate_goal)
                    self.agents[goal_idx].goal = goal.copy()
                    break
            else:
                raise RuntimeError(
                    f'Failed to place goal {goal_idx} after {self.sample_retry_limit} attempts.'
                )
    ...
    def step(self, actions):
        ...
        for idx, agent in enumerate(self.agents):
            ...
            if not agent.reached and current_dist <= self.goal_tolerance:
                agent.reached = True
                agent.vel[:] = 0.0
                rewards[idx] += 8.0

            dones[idx] = agent.reached
    ...
    def summary(self):
        ...
        return {
            ...
            "win_tag": bool(all(agent.reached for agent in self.agents)),
        }
    ...
    def _render_3d(self, ax):
        ...
        for idx, goal in enumerate(self.goals):
            color = self._agent_color(idx)
            goal_z = goal.pos[2] if self.dim_actions == 3 else 0.0
            ax.scatter(
                goal.pos[0],
                goal.pos[1],
                goal_z,
...
class UAVEnvDiscreteWrapper:
    ...
    def step(self, actions):
        ...
        battle_won = bool(all(agent.reached for agent in self.env.agents))
